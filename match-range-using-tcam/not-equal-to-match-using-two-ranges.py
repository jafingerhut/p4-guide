...
parser.add_argument('--value', dest='value', type=auto_int, required=True,
                    help="""The value to match not equal to against.  Interpreted as Python literal number, allowing 0x prefix for base 16, 0o for base 8, and 0b for base 2.""")
args = parser.parse_known_args()[0]

debug = args.debug
#print("debug=%s" % (debug))
#print('W=%d' % (args.W))
#print('value=%d' % (args.value))
...
def not_equal_tcam_entries(W, val):
    if type(W) is not int:
        msg = ("W must be int but got %s" % (type(W)))
        raise TypeError(msg)
    if type(val) is not int:
        msg = ("val must be int but got %s" % (type(val)))
        raise TypeError(msg)

    max_field_val = (1 << W) - 1

    if val < 0 or val > max_field_val:
        msg = ("Error: val=%d outside of range of %d-bit value [0, %d]"
               "" % (val, W, max_field_val))
        raise ValueError(msg)

    entries1 = []
    if val > 0:
        entries1 = range_to_tcam_entries(W, 0, val - 1)
        check_tcam_entries(W, 0, val - 1, entries1)
    entries2 = []
    if val < max_field_val:
        entries2 = range_to_tcam_entries(W, val + 1, max_field_val)
        check_tcam_entries(W, val + 1, max_field_val, entries2)
    return (entries1 + entries2)


entries = not_equal_tcam_entries(args.W, args.value)
debug_print_tcam_entries(args.W, entries)

# Exhaustive test of all ranges that fit in width W bits
if args.test:
    hist = collections.defaultdict(int)
    for val in range(1 << args.W):
        entries = not_equal_tcam_entries(args.W, val)
        num_entries = len(entries)
        hist[num_entries] += 1
        
    print("Number of TCAM entries required to implement not-equal-to, implemented via two ranges (or one for the extreme values):")
    print("# entries   # values using that number of entries")
    print("----------  -------------------------------------")
    for num_entries in sorted(hist.keys()):
        print("%10d  %4d" % (num_entries, hist[num_entries]))

...
def print_select_field_code(
    ...
    for output_field in range(output_fields-1, 0-1, -1):
        decls.append("    bit<%d> out%d;\n" % (field_width, output_field))
        ...
        for input_field in range(input_fields-1, 0-1, -1):
            ...
            # 'else'.
            if_cond = ("if (select_control[%d:%d] == %d)"
                       "" % (sc_msb, sc_lsb, input_field))
            if input_field == 0:
                if_cond = ""

            out_assign += ("""        %s%s {
            out%d = data_in[%d:%d];
""" % (else_kw, if_cond, output_field, data_in_msb, data_in_lsb))
        out_assign += "        }\n"

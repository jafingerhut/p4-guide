...
control verifyChecksum(in headers hdr,
                       inout metadata meta) {
...
control computeChecksum(inout headers hdr,
    ...
    Checksum16() inner_ipv4_checksum;
    Checksum16() ipv4_checksum;
    Checksum16() outer_ipv4_checksum;
    apply {
        if (hdr.inner_ipv4.isValid()) {
            hdr.inner_ipv4.hdrChecksum =
                inner_ipv4_checksum.get({ hdr.inner_ipv4.version,
                            hdr.inner_ipv4.ihl,
                            hdr.inner_ipv4.diffserv,
                            hdr.inner_ipv4.totalLen,
                            hdr.inner_ipv4.identification,
        ...
        if (hdr.outer_ipv4.isValid()) {
            hdr.outer_ipv4.hdrChecksum =
                outer_ipv4_checksum.get({ hdr.outer_ipv4.version,
                            hdr.outer_ipv4.ihl,

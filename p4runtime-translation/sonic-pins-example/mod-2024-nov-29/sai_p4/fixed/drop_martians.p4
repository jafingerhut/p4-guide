...
#define IS_MULTICAST_IPV6(address) \
  ((bit<128>) address & (bit<128>) IPV6_MULTICAST_MASK == (bit<128>) IPV6_MULTICAST_VALUE)

#define IS_LOOPBACK_IPV6(address) \
  ((bit<128>) address & (bit<128>) IPV6_LOOPBACK_MASK == (bit<128>) IPV6_LOOPBACK_VALUE)

#define IS_MULTICAST_IPV4(address) \
  ((bit<32>) address & (bit<32>) IPV4_MULTICAST_MASK == (bit<32>) IPV4_MULTICAST_VALUE)

#define IS_BROADCAST_IPV4(address) \
  ((bit<32>) address == (bit<32>) IPV4_BROADCAST_VALUE)

#define IS_LOOPBACK_IPV4(address) \
  ((bit<32>) address & (bit<32>) IPV4_LOOPBACK_MASK == (bit<32>) IPV4_LOOPBACK_VALUE)

// I/G bit = 1 means multicast.
#define IS_UNICAST_MAC(address) \
  (((bit<48>) address)[40:40] == 0)

#define IS_IPV4_MULTICAST_MAC(address) \
  (((bit<48>) address)[47:24] == 0x01005E && ((bit<48>) address)[23:23] == 0)

#define IS_IPV6_MULTICAST_MAC(address) \
  (((bit<48>) address)[47:32] == 0x3333)

                    ...
                    inout standard_metadata_t stdmeta)
{
    ...
    table t1 {
        key = {
            // This does not compile with p4c as of 2020-Mar, because
            // both values added have types declared with `type`
            // keyword, and currently arithmetic is not supported on
            // them.

            // Suppose you wish it were legal to write this expression
            // without any casts at all.
            
            // What should the resulting type be in the control plane
            // API, and what rule should the compiler use to determine
            // it?
            hdr.andycustom.srcAddr + hdr.ipv4.srcAddr: exact;
        }
        actions = {
            set_output;
            my_drop;

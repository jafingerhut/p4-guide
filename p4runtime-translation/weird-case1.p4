                    ...
                    inout standard_metadata_t stdmeta)
{
    ...
    table t1 {
        key = {
            // 2020-Mar p4c knows that this is type IPv4Addr_t, and
            // says so in the auto-genearted P4Info file.
            lookupAddr: exact;
        }
        actions = {
            set_output;
    ...
    apply {
        if (hdr.ethernet.isValid() && hdr.andycustom.isValid() && hdr.ipv4.isValid()) {
            lookupAddr = (IPv4Addr_t) (((bit<32>) hdr.andycustom.srcAddr) +

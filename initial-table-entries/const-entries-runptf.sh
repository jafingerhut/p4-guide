...
set -x
p4test \
    --p4runtime-entries-files const-entries-bmv2.entries.txtpb \
    const-entries-bmv2.p4

p4c --target bmv2 \
    --arch v1model \
    --p4runtime-files const-entries-bmv2.p4info.txtpb \
    const-entries-bmv2.p4
...
sudo ${P4_EXTRA_SUDO_OPTS} `which ptf` \
    ...
    -i 6@veth13 \
    -i 7@veth15 \
    --test-params="grpcaddr='localhost:9559';p4info='const-entries-bmv2.p4info.txtpb';config='const-entries-bmv2.json'" \
    --test-dir .

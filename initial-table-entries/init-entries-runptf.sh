...
set -x
p4test \
    --p4runtime-entries-files init-entries-bmv2.entries.txtpb \
    init-entries-bmv2.p4

p4c --target bmv2 \
    --arch v1model \
    --p4runtime-files init-entries-bmv2.p4info.txtpb \
    init-entries-bmv2.p4
...
sudo ${P4GUIDE_SUDO_OPTS} `which ptf` \
    ...
    -i 6@veth13 \
    -i 7@veth15 \
    --test-params="grpcaddr='localhost:9559';p4info='init-entries-bmv2.p4info.txtpb';config='init-entries-bmv2.json'" \
    --test-dir .
